...
class transition(nn.Module):
    ...
    def forward(self, x):
        ...
        output = self.pool(output)  # 特征图大小减半
        return output
...
class clique_block(nn.Module):
    def __init__(self, input_channels, channels_per_layer, layer_num, loop_num, keep_prob):
        ...
        self.conv_param = nn.ModuleList([nn.Conv2d(self.channels_per_layer, self.channels_per_layer, kernel_size=1, padding=0, bias=False)
                                   for i in range((self.layer_num + 1) ** 2)])  # init conv_param

        for i in range(1, self.layer_num + 1):
            self.conv_param[i] = nn.Conv2d(self.input_channels, self.channels_per_layer, kernel_size=1, padding=0, bias=False) # modify conv_param[i]
        for i in range(1, self.layer_num + 1):
