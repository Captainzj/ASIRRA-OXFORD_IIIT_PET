...
def show_cam_on_image(img_path, mask):
	...
	cam = cam / np.max(cam)
	saved_filepath = os.path.join(img_path.split('.')[0]+'_shallow_densenet_100_GRAD_CAM.png')
...
if __name__ == '__main__':
	...
	# model = densenet_DIY_64()
	# model = densenet_DIY_100()
	# model = densenet_DIY_100_f()

	# load the pre-saved model
	saved_model = 'Results/model/DenseNet/densenet_DIY/depth_40_k_48/'
	try:
	...
	# target_layer_names = ["classifier"]
	grad_cam = GradCam(model, target_layer_names, use_cuda=args.use_cuda)

	# Input (image)
	dir_path = '/home/captain/Desktop/test-5-24/'
	images = []
